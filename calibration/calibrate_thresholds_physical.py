...
PHYSICAL chemistry questions through both solvers.

Subject-specific changes vs. organic's calibrate_thresholds.py:
  - 6 archetypes (I-VI) instead of 4 (I-IV)
  - topic filter matches "Physical Chemistry" instead of organic/carbon
...
Output: calibration_results_physical.json + printed statistics
"""
...
OUT_PATH = os.path.join(_HERE, "calibration_results_physical.json")

SUBJECT_TOPIC_MATCH = "physical"   # matches "Physical Chemistry" topic field

# ── 6 physical archetypes (labels match what the questions actually do) ───────
...
# fallback below maps those onto the nearest physical bucket so a re-run can
# still stratify a sample.
PHYSICAL_ARCHETYPES = [
    ("I",   "Formula Application & Numerical Computation"),
    ("II",  "Multi-Concept / Multi-Chapter Integration"),
    ("III", "Graphical Interpretation"),
    ("IV",  "Comparative Property & Trend Reasoning"),
    ("V",   "Conceptual & Postulate-Based Reasoning"),
    ("VI",  "Applied / Real-World"),
]

RAW_ARCHETYPE_FALLBACK = {
    "counting_enumeration":       "I",
    "long_reaction_chains":       "II",
    "comparative_ranking":        "IV",
    "deep_mechanistic_reasoning": "V",
}
...
ARCHETYPE_MAP = {code: f"{name} ({code})" for code, name in PHYSICAL_ARCHETYPES}
CODE_LOOKUP = {}
for code, name in PHYSICAL_ARCHETYPES:
...
def stratified_sample(qs):
    by_arch = {code: [] for code, _ in PHYSICAL_ARCHETYPES}
    for q in qs:
...
def strong_prompt(q):
    return f"""You are an expert physical chemist solving a JEE Advanced question.
Work through the problem step by step using rigorous quantitative/conceptual reasoning
(formulae, derivations, graphical trends as needed) before committing to a final answer.
...
def run_question(q, idx, total):
    ...
    try:
        resp = sambanova_call(lambda: strong_client.chat.completions.create(
            model=STRONG_MODEL,
            messages=[
                {"role": "system", "content": "You are an expert physical chemist. Output JSON only."},
...
def main():
    print("=" * 65)
    print("  JEEGen Threshold Calibration — PHYSICAL CHEMISTRY")
    print("  Archetypes: 6 (I-VI)")
    print("  Strong: DeepSeek-V3.2 (SambaNova)")
    print("  Weak  : llama3.2 (Ollama)")
    print("=" * 65)

    print("\nLoading JEE Advanced physical questions...")
    all_qs = load_physical_questions()
