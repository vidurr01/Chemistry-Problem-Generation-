"""
Run strong solver on all 143 inorganic seeds to calibrate:
  - model_solution_length  (trace word count, per-archetype)
  - distractor_plausibility (rejected-path count, per-archetype)

Fills the PENDING_SOLVER_TRACES entries in meta_tag_norm_stats_inorganic.json.
...
Subject-specific changes vs. organic's calibrate_solver_metatags.py:
  ...
  - default fallback archetype is "III" (Structure/Bonding), the largest
    inorganic archetype bucket (50/143), mirroring organic's III default
...
Saves incremental progress to solver_traces_inorganic.json so restarts are safe.
"""
...
TRACES_FILE = os.path.join(_HERE, "solver_traces_inorganic.json")
SEEDS_FILE  = os.path.join(_HERE, "..", "data", "seeds", "jeeadv_inorganic_seeds.json")
NORM_FILE   = os.path.join(_HERE, "..", "data", "seeds", "meta_tag_norm_stats_inorganic.json")

DEFAULT_ARCH_CODE = "III"   # largest bucket (Structure/Bonding/Electronic Config), 50/143

ARCH_CODE_MAP = {
    "Reaction & Transformation Chains": "I",
    "Property / Trend Ordering & Exception Reasoning": "II",
    "Structure, Bonding & Electronic Configuration Reasoning": "III",
    "Numerical / Computational": "IV",
    "Qualitative Identification (Observation-Based Deduction)": "V",
    "Applied / Real-World Conceptual Reasoning": "VI",
...
def run_solver(question_text: str, archetype: str) -> dict:
    prompt = f"""You are an expert inorganic chemist. Solve this problem rigorously, showing full
structural/periodic-trend/reaction reasoning step by step.
...
def main():
    ...
    for code in ARCH_CODES:
        ...
        # clear the PENDING status now that traces exist
        if "number_of_exceptions" in pa and pa["number_of_exceptions"].get("status") == "PENDING_SOLVER_TRACES":
            pa["number_of_exceptions"].pop("status", None)
